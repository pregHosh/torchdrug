...
class PropertyPrediction(tasks.Task, core.Configurable):
    ...
    def __init__(
        ...
        # modify so that during interference, dummy dataset is not needed
        # but need specifying task_names, and std_mean manually
        self.mlp = None
        if std_mean:
            self.std = std_mean[0]
...
class PropertyPrediction_local(tasks.Task, core.Configurable):
    ...
    def predict(self, batch, all_loss=None, metric=None, evaluate=False):
        ...
        def get_adj_idxs(batch, n_atoms_csum):
            idx_cols = list(filter(lambda i: "idx" in i, batch))
            # TODO maybe couldve been better written
            idxs_adj = []
            for col in idx_cols:
                idx_i = batch[col]
                if n_atoms_csum[-1] == 0:
