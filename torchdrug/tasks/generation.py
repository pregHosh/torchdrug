...
from torchdrug import core, data, layers, metrics, tasks, transforms
...
class AutoregressiveGeneration(tasks.Task, core.Configurable):
    ...
    def reinforce_forward(self, batch):
        ...
        # NOTE we can convert these to smiles
        ...
        if len(graph) == 0 or graph.num_nodes.max() == 1:
            ...
            for task in self.task:
                if task == "plogp":
                    ...
                    metric["QED"] = nan
                    metric["QED (max)"] = nan
                # NOTE assume additional tasks are function/class
                else:
        ...
        reward = (
            reward
            * self.gamma
            ** (masked_graph.original_num_nodes - masked_graph.num_nodes).float()
        )

        # per graph size reward baseline
        weight = torch.ones_like(masked_graph.num_nodes, dtype=torch.float)
        baseline = scatter_add(
            reward, masked_graph.num_nodes, dim_size=self.max_node + 1
        ) / (
            scatter_add(weight, masked_graph.num_nodes, dim_size=self.max_node + 1)
            + self.eps
        )
        self.node_baseline = self.node_baseline * self.baseline_momentum + baseline * (
            1 - self.baseline_momentum
        )
        ...
        log_likelihood = self.node_model(
            masked_graph, node_target, None, all_loss, metric
        )
        agent_log_likelihood = self.agent_node_model(
            masked_graph, node_target, None, all_loss, metric
        )
        objective = functional.clipped_policy_gradient_objective(
            log_likelihood, agent_log_likelihood, reward
        )
        ...
        self.edge_baseline = self.edge_baseline * self.baseline_momentum + baseline * (
            1 - self.baseline_momentum
        )
        ...
        log_likelihood = self.edge_model(
            masked_graph, edge_target, edge, all_loss, metric
        )
        agent_log_likelihood = self.agent_edge_model(
            masked_graph, edge_target, edge, all_loss, metric
        )
        objective = functional.clipped_policy_gradient_objective(
            log_likelihood, agent_log_likelihood, reward
    ...
    @torch.no_grad()
    def generate(
        self, num_sample, max_resample=20, off_policy=False, early_stop=False, verbose=0
    ):
        ...
        for node_in in range(self.max_node):
            ...
            graph = node_graph = data.PackedMolecule(
                edge_list,
                atom_type,
                edge_list[:, -1],
                num_nodes,
            ...
            for node_out in range(start, node_in):
                ...
                edge = torch.tensor([node_in, node_out], device=self.device).repeat(
                    num_sample, 1
                )
                # default: non-edge
                bond_pred = (self.num_bond_type - 1) * torch.ones(
                    num_sample, dtype=torch.long, device=self.device
                )
                for i in range(max_resample):
                    ...
                    tmp_edge_list, tmp_num_edges = self._append(
                        edge_list, num_edges, edge_pred, mask
                    )
                    edge_pred = torch.cat(
                        [edge.flip(-1), bond_pred.unsqueeze(-1)], dim=-1
                    )
                    tmp_edge_list, tmp_num_edges = self._append(
                        tmp_edge_list, tmp_num_edges, edge_pred, mask
                    )
                    tmp_graph = data.PackedMolecule(
                        tmp_edge_list,
                        self.id2atom[atom_type],
                        tmp_edge_list[:, -1],
                        num_nodes,
                        tmp_num_edges,
                        num_relation=num_relation,
                ...
                if not is_valid.all() and verbose:
                    ...
                    logger.warning(
                        "edge (%d, %d): %d / %d molecules are invalid even after %d resampling"
                        % (node_in, node_out, num_invalid, num_working, max_resample)
                    )

                mask = (bond_pred < edge_model.input_dim - 1) & ~completed
                edge_pred = torch.cat([edge, bond_pred.unsqueeze(-1)], dim=-1)
                edge_list, num_edges = self._append(
                    edge_list, num_edges, edge_pred, mask
                )
            ...
            if node_in > 0:
                assert (
                    graph.num_edges[completed] == node_graph.num_edges[completed]
                ).all()
    ...
    def _append(self, data, num_xs, input, mask=None):
        ...
        new_data = torch.zeros(
            new_num_x, *data.shape[1:], dtype=data.dtype, device=data.device
        )
    ...
    def mask_node(self, graph, metric=None):
        if self.num_node_sample == -1:
            ...
            if metric is not None:
                metric["node mask / graph"] = torch.tensor(
                    [len(masked_graph) / len(graph)], device=graph.device
                )
        else:
    ...
    def mask_edge(self, graph, metric=None):
        if self.num_edge_sample == -1:
            ...
            if metric is not None:
                metric["edge mask / graph"] = torch.tensor(
                    [len(masked_graph) / len(graph)], device=graph.device
                )
        else:
            masked_graph, edge_target, edge = self.sample_edge(
                graph, self.num_edge_sample
            )
    ...
    def sample_node(self, graph, num_sample):
        ...
        num_keep_nodes = (
            torch.rand(len(graph), device=graph.device) * num_nodes
        )  # [0, num_nodes)
    ...
    def sample_edge(self, graph, num_sample):
        if (graph.num_nodes < 2).any():
            ...
            warnings.warn(
                "Graphs with less than 2 nodes can't be used for edge generation learning. Dropped"
            )
        ...
        num_node2num_dense_edge = (
            torch.arange(num_max_node + 1, device=graph.device) ** 2
        )
        num_node2length_idx = (
            lengths.unsqueeze(-1) < num_node2num_dense_edge.unsqueeze(0)
        ).sum(dim=0)
        # uniformly sample a mask from each graph's valid masks
        length_indexes = (
            torch.rand(len(graph), device=graph.device)
            * num_node2length_idx[graph.num_nodes]
        )
        ...
        positive_graph = scatter_add(
            positive_edge.long(), graph.edge2graph, dim=0, dim_size=len(graph)
        ).bool()
        # default: non-edge
        target = (self.num_bond_type - 1) * torch.ones(
            graph.batch_size, dtype=torch.long, device=graph.device
        )
    ...
    def _valid_edge_prefix_lengths(self, graph):
        ...
        lengths = lengths[
            (num_edge_unrolls <= self.max_edge_unroll) & (num_edge_unrolls > 0)
        ]
...
class GCPNGeneration(tasks.Task, core.Configurable):
    ...
    def __init__(
        ...
        self.new_atom_embeddings = nn.Parameter(
            torch.zeros(self.id2atom.size(0), self.model.output_dim)
        )
        nn.init.normal_(self.new_atom_embeddings, mean=0, std=0.1)
        self.inp_dim_stop = self.model.output_dim
        self.mlp_stop = layers.MultiLayerPerceptron(
            self.inp_dim_stop, [self.hidden_dim_mlp, 2], activation="tanh"
        )

        self.inp_dim_node1 = self.model.output_dim + self.model.output_dim
        self.mlp_node1 = layers.MultiLayerPerceptron(
            self.inp_dim_node1, [self.hidden_dim_mlp, 1], activation="tanh"
        )
        self.inp_dim_node2 = 2 * self.model.output_dim + self.model.output_dim
        self.mlp_node2 = layers.MultiLayerPerceptron(
            self.inp_dim_node2, [self.hidden_dim_mlp, 1], activation="tanh"
        )
        self.inp_dim_edge = 2 * self.model.output_dim
        self.mlp_edge = layers.MultiLayerPerceptron(
            self.inp_dim_edge,
            [self.hidden_dim_mlp, self.model.num_relation],
            activation="tanh",
        )
    ...
    def predict(self, graph, label_dict, use_agent=False):
        ...
        extended_node2graph = (
            torch.arange(graph.num_nodes.size(0), device=self.device)
            .unsqueeze(1)
            .repeat([1, self.id2atom.size(0)])
            .view(-1)
        )  # (num_graph * 16)
        extended_node2graph = torch.cat(
            (graph.node2graph, extended_node2graph)
        ...
        if not use_agent:
            node1_logits = self.mlp_node1(node1_feature).squeeze(
                1
            )  # (num_node + 16 * num_graph)
        else:
            node1_logits = self.agent_mlp_node1(node1_feature).squeeze(
                1
            )  # (num_node + 16 * num_graph)
        ...
        node1_index_per_graph = (graph.num_cum_nodes - graph.num_nodes) + label_dict[
            "label1"
        ]  # (num_graph)
        node1_index = node1_index_per_graph[
            extended_node2graph
        ]  # (num_node + 16 * num_graph)
        node2_feature_node1 = node1_feature[
            node1_index
        ]  # (num_node + 16 * num_graph, n_out)
        node2_feature = torch.cat(
            (node2_feature_node1, node2_feature_node2), 1
        )  # (num_node + 16 * num_graph, 2n_out)
        if not use_agent:
            node2_logits = self.mlp_node2(node2_feature).squeeze(
                1
            )  # (num_node + 16 * num_graph)
        else:
            node2_logits = self.agent_mlp_node2(node2_feature).squeeze(
                1
            )  # (num_node + 16 * num_graph)
        ...
        is_new_node = (
            label_dict["label2"] - graph.num_nodes
        )  # if an entry is non-negative, this is a new added node. (num_graph)
        graph_offset = torch.arange(graph.num_nodes.size(0), device=self.device)
        node2_index_per_graph = torch.where(
            is_new_node >= 0,
            graph.num_node + graph_offset * self.id2atom.size(0) + is_new_node,
            label_dict["label2"] + graph.num_cum_nodes - graph.num_nodes,
        )  # (num_graph)
        node2_index = node2_index_per_graph[extended_node2graph]

        edge_feature_node1 = node2_feature_node2[
            node1_index_per_graph
        ]  # (num_graph, n_out)
        edge_feature_node2 = node2_feature_node2[
            node2_index_per_graph
        ]  # #(num_graph, n_out)
        edge_feature = torch.cat(
            (edge_feature_node1, edge_feature_node2), 1
        )  # (num_graph, 2n_out)
        if not use_agent:
    ...
    def reinforce_forward(self, batch):
        ...
        stop_graph, stop_label1, stop_label2, stop_label3, stop_label4 = self.all_stop(
            graph
        )
        edge_graph, edge_label1, edge_label2, edge_label3, edge_label4 = self.all_edge(
            graph
        )
        ...
        label_dict = {
            "label1": label1_target,
            "label2": label2_target,
            "label3": label3_target,
        ...
        reward = graph.reward
        reward = (
            reward * self.gamma ** (graph.original_num_nodes - graph.num_nodes).float()
        )
        ...
        self.moving_baseline = (
            self.moving_baseline * self.baseline_momentum
            + baseline * (1 - self.baseline_momentum)
        )
        reward -= self.moving_baseline[graph.num_nodes]
        reward += graph.is_valid

        # calculate object
        stop_logits, node1_logits, node2_logits, edge_logits, index_dict = self.predict(
            graph, label_dict
        )
        with torch.no_grad():
            (
                ...
                old_edge_logits,
                old_index_dict,
            ) = self.predict(graph, label_dict, use_agent=True)

        stop_prob = F.log_softmax(stop_logits, dim=-1)
        node1_prob = scatter_log_softmax(
            node1_logits, index_dict["extended_node2graph"]
        )
        node2_prob = scatter_log_softmax(
            node2_logits, index_dict["extended_node2graph"]
        )
        edge_prob = F.log_softmax(edge_logits, dim=-1)
        old_stop_prob = F.log_softmax(old_stop_logits, dim=-1)
        old_node1_prob = scatter_log_softmax(
            old_node1_logits, old_index_dict["extended_node2graph"]
        )
        old_node2_prob = scatter_log_softmax(
            old_node2_logits, old_index_dict["extended_node2graph"]
        )
        old_edge_prob = F.log_softmax(old_edge_logits, dim=-1)

        cur_logp = (
            stop_prob[:, 0]
            + node1_prob[index_dict["node1_index_per_graph"]]
            + node2_prob[index_dict["node2_index_per_graph"]]
            + torch.gather(edge_prob, -1, label3_target.view(-1, 1)).view(-1)
        )
        cur_logp[label4_target == 1] = stop_prob[:, 1][label4_target == 1]

        old_logp = (
            old_stop_prob[:, 0]
            + old_node1_prob[old_index_dict["node1_index_per_graph"]]
            + old_node2_prob[index_dict["node2_index_per_graph"]]
            + torch.gather(old_edge_prob, -1, label3_target.view(-1, 1)).view(-1)
        )
        old_logp[label4_target == 1] = old_stop_prob[:, 1][label4_target == 1]
        objective = functional.clipped_policy_gradient_objective(
            cur_logp, old_logp, reward
        )
        objective = objective.mean()
    ...
    def MLE_forward(self, batch):
        ...
        loss_stop = F.nll_loss(
            F.log_softmax(stop_logits, dim=-1), label4_target, reduction="none"
        )
        loss_stop = 0.5 * (
            torch.mean(loss_stop[label4_target == 0])
            + torch.mean(loss_stop[label4_target == 1])
        )
        ...
        loss_node1 = -(
            scatter_log_softmax(node1_logits, index_dict["extended_node2graph"])[
                index_dict["node1_index_per_graph"]
            ]
        )
        ...
        loss_node2 = -(
            scatter_log_softmax(node2_logits, index_dict["extended_node2graph"])[
                index_dict["node2_index_per_graph"]
            ]
        )
        ...
        loss_edge = F.nll_loss(
            F.log_softmax(edge_logits, dim=-1), label3_target, reduction="none"
        )
    ...
    def _construct_dist(self, prob_, graph):
        ...
        start_extend = torch.arange(len(self.id2atom), device=self.device).repeat(
            graph.num_nodes.size()
        )  # (num_graph * 16)
        index_extend = (
            torch.arange(len(graph), device=self.device) * max_size + graph.num_nodes
        )
        index2graph = torch.arange(len(graph), device=self.device).repeat_interleave(
            len(self.id2atom)
        )
    ...
    def _sample_action(self, graph, off_policy):
        ...
        extended_node2graph = torch.arange(
            len(graph), device=self.device
        ).repeat_interleave(
            len(self.id2atom)
        )  # (num_graph * 16)
        extended_node2graph = torch.cat(
        ...
        node1_feature = torch.cat(
            (node1_feature, new_atom_embeddings.repeat([graph.num_nodes.size(0), 1])), 0
        )  # (num_node + 16 * num_graph, n_out)
        node2_feature_node2 = (
            node1_feature.clone()
        )  # (num_node + 16 * num_graph, n_out)
        ...
        node1_logits = mlp_node1(node1_feature).squeeze(
            1
        )  # (num_node + 16 * num_graph)
        # mask the extended part
        mask = torch.zeros(node1_logits.size(), device=self.device)
        ...
        node1_prob = scatter_softmax(
            node1_logits, extended_node2graph
        )  # (num_node + 16 * num_graph)
        node1_prob_dist, tmp = self._construct_dist(
            node1_prob, graph
        )  # (num_graph, max)
        ...
        node2_logits = mlp_node2(node2_feature).squeeze(
            1
        )  # (num_node + 16 * num_graph)
        ...
        node2_prob = scatter_softmax(
            node2_logits, extended_node2graph
        )  # (num_node + 16 * num_graph)
        node2_prob_dist, tmp = self._construct_dist(
            node2_prob, graph
        )  # (num_graph, max)
    ...
    def _top1_action(self, graph, off_policy):
        ...
        node1_index_per_graph = scatter_max(node1_logits, extended_node2graph)[
            1
        ]  # (num_node + 16 * num_graph)
        ...
        node2_index_per_graph = scatter_max(node2_logits, extended_node2graph)[
            1
        ]  # (num_node + 16 * num_graph)

        is_new_node = (
            node2_index_per_graph - graph.num_node
        )  # non negative if is new node
        graph_offset = torch.arange(graph.num_nodes.size(0), device=self.device)
        node2_pred = torch.where(
            is_new_node >= 0,
            graph.num_nodes + is_new_node - graph_offset * self.id2atom.size(0),
    ...
    def _apply_action(self, graph, off_policy, max_resample=10, verbose=0, min_node=5):
        ...
        for i in range(max_resample):
            ...
            atom_type, num_nodes = functional._extend(
                graph.atom_type, graph.num_nodes, new_atom_type, has_new_node
            )
            ...
            is_modified_edge = (edge_list[:, :2] == new_edge[graph.edge2graph]).all(
                dim=-1
            ) & (stop_action[graph.edge2graph] == 0)
            has_modified_edge = (
                scatter_max(
                    is_modified_edge.long(), graph.edge2graph, dim_size=len(graph)
                )[0]
                > 0
            )
            ...
            new_edge_list = torch.stack(
                [node1_action, node2_action, edge_action], dim=-1
            )[has_new_edge]
            bond_type = functional._extend(
                bond_type, graph.num_edges, edge_action[has_new_edge], has_new_edge
            )[0]
            edge_list, num_edges = functional._extend(
                edge_list, graph.num_edges, new_edge_list, has_new_edge
            )

            # tmp add reverse edges
            new_edge_list = torch.stack(
                [node2_action, node1_action, edge_action], dim=-1
            )[has_new_edge]
            bond_type = functional._extend(
                bond_type, num_edges, edge_action[has_new_edge], has_new_edge
            )[0]
            edge_list, num_edges = functional._extend(
                edge_list, num_edges, new_edge_list, has_new_edge
            )

            tmp_graph = type(graph)(
                edge_list,
                atom_type=atom_type,
                bond_type=bond_type,
                num_nodes=num_nodes,
        ...
        if not is_valid.all() and verbose:
            ...
            logger.warning(
                "%d / %d molecules are invalid even after %d resampling"
        ...
        for k, v in data_dict.items():
            if "node" in meta_dict[k]:
                ...
                data_dict[k] = functional._extend(
                    v, graph.num_nodes, new_data, has_new_node
                )[0]
            if "edge" in meta_dict[k]:
                shape = (len(new_edge_list) * 2, *v.shape[1:])
                new_data = torch.zeros(shape, dtype=v.dtype, device=self.device)
                data_dict[k] = functional._extend(
                    v, graph.num_edges, new_data, has_new_edge * 2
                )[0]
    ...
    def _update_molecule_feature(self, graphs):
        ...
        new_graphs = type(graphs).from_molecule(
            mols, kekulize=True, atom_feature="symbol"
        )

        node_feature = torch.zeros(
            graphs.num_node,
            *new_graphs.node_feature.shape[1:],
            dtype=new_graphs.node_feature.dtype,
            device=graphs.device,
        )
        edge_feature = torch.zeros(
            graphs.num_edge,
            *new_graphs.edge_feature.shape[1:],
    ...
    def generate(
        ...
        graph = data.Molecule.from_smiles(
            initial_smiles, kekulize=True, atom_feature="symbol"
        ).repeat(num_sample)
        ...
        for i in range(max_step):
            ...
            if i == max_step - 1:
                ...
                result.append(
                    new_graph[
                        new_graph.is_stopped | (new_graph.num_nodes == (self.max_node))
                    ]
                )

                is_continue = (~new_graph.is_stopped) & (
                    new_graph.num_nodes < (self.max_node)
                )
    ...
    def all_stop(self, graph):
        if (graph.num_nodes < 2).any():
            graph = graph[graph.num_nodes >= 2]
            warnings.warn(
                "Graphs with less than 2 nodes can't be used for stop prediction learning. Dropped"
            )
        ...
        return (
            ...
            label2,
            label3,
            torch.ones(len(graph), dtype=torch.long, device=self.device),
        )
    ...
    def all_edge(self, graph):
        ...
        masked_undirected_edge_id = torch.where(
            edge_mask,
            undirected_edge_id,
            -torch.ones(
                undirected_edge_id.size(), dtype=torch.long, device=graph.device
            ),
        )
        current_circum_box_size = scatter_max(
            masked_undirected_edge_id, graph.edge2graph, dim=0
        )[0]
        ...
        # default: non-edge
        target = (self.model.num_relation) * torch.ones(
            graph.batch_size, dtype=torch.long, device=graph.device
        )
